...
const NewProjectsExtended = ({setPopUp,show,FacultyNames,requirements,title,description,team,projectId}) => {
  ...
  async function apply(id) {
    try{
      const response = await axios.post(
         `http://${import.meta.env.localhost2}:3000/projectRoutes/applyforProject`,
        {
          projectId:id
        },
        {
  ...
  return ReactDOM.createPortal(
    ...
    <div className="min-w-[90vw] mx-auto p-6 bg-white rounded-lg shadow-lg min-h-[80vh]">
      ...
      <div className="text-right">
            ...
            <span className='bg-red-200 text-red-700 font-bold  px-4 py-4 rounded-lg '>Already applied</span>
          ) : (
            <button className="bg-blue-500 text-white font-bold cursor-pointer px-4 py-4 rounded-lg hover:bg-blue-700" onClick={()=>apply(projectId)}>
            Request to Join 
          </button>
          )}
      </div>
    ...
    document.querySelector(".NewProjectsExtended")
  );
};

export default NewProjectsExtended;
