...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchFacultyProflie = async () => {
      try {
        const response = await axios.get(`http://${import.meta.env.localhost2}:3000/studentsRoutes/studentProfile`, {
          headers: {
...
useEffect(() => {
  const fetchAllProjects = async () => {
    try {
      const response = await axios.get(`http://${import.meta.env.localhost2}:3000/projectRoutes/getAllProjects`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("student_token")}`
        }
      });
...
useEffect(() => {
  const fetchFaculty = async () => {
    try {
      const response = await axios.get(`http://${import.meta.env.localhost2}:3000/facultyRoutes/getAllFaculty`, {
        headers: {
  ...
  const fetchCurrentStudent = async () => {
    try {
      const response = await axios.get(`http://${import.meta.env.localhost2}:3000/studentsRoutes/getCurrentProject`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("student_token")}`
        }
      });
  ...
  const fetchAllStudents = async () => {
    try {
      const response = await axios.get(`http://${import.meta.env.localhost2}:3000/studentsRoutes/getAllStudents`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("student_token")}`
        }
      });
