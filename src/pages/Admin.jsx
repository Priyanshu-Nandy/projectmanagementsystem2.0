...
useEffect(() => {
  const fetchFacultyProflie = async () => {
    try {
      const response = await axios.get(`http://${import.meta.env.localhost2}:3000/facultyRoutes/facultyProfile`, {
        headers: {
  ...
  fetchFacultyProflie();
          ...
          try {
            const response = await axios.get(`http://${import.meta.env.localhost2}:3000/projectRoutes/getallapplications`, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("access_token")}`
              }
            });
      ...
      useEffect(() => {
        const fetchProjects = async () => {
          try {
            const response = await axios.get(`http://${import.meta.env.localhost2}:3000/projectRoutes/getProjects`, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("access_token")}`
              }
            });
